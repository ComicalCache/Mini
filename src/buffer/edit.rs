use crate::{cursor, document::Document, viewport::Viewport};
use std::borrow::Cow;
...
pub fn write_char(doc: &mut Document, view: &mut Viewport, ch: char) {
    doc.write_char(ch);
...
pub fn write_new_line_char(doc: &mut Document, view: &mut Viewport) {
...
pub fn write_tab(doc: &mut Document, view: &mut Viewport) {
    doc.write_str(TAB);
...
pub fn delete_char(doc: &mut Document, view: &mut Viewport) {
    let cur = doc.cur;
